...
def game_loop():
    ...
    while bank > 0:
        if current_bet > 0 and bank >= current_bet:
            bet_r = input("\n PRESS ENTER TO REPEAT BET, 'NO' TO PLACE NEW ONE: ") #removed unneccasry bracket from the word enter
            if bet_r == "" or bet_r == "Y":
                bet = current_bet
            else:
                current_bet = 0
...
def start():
    ...
    command = input("YOU START WITH $5,000! \n\nPRESS ENTER TO BEGIN ") #removed unnecceasy brackets on enter from original #updated 10k to 5k
    if command == "":
        bank = 5000 #updated bank number
        game_loop()
    else:
